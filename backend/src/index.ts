...
app.post("/api/campaigns", async (req: Request, res: Response) => {
  try {
    ...
    // Upsert User to ensure FK integrity
    // This ensures the User exists before creating the campaign (prevents P2003 foreign key errors)
    // Use email as unique key since it's guaranteed to be unique and works for both new and existing users
    const dbUser = await prisma.user.upsert({
      where: { email: clerkEmail },
      update: {}, // No update needed if user exists
      create: {
        email: clerkEmail,
        // id will be auto-generated by Prisma (CUID) - this matches existing users
      },
    });

    console.log('[POST /api/campaigns] User upserted - id:', dbUser.id, 'email:', dbUser.email);
    
    const userId = dbUser.id; // Use the database User id (from upsert)
