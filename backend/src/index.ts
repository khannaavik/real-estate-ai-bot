...
import { getNextValidCallTime, formatNextCallTime, isWithinBatchCallWindow, isWithinCallWindow } from "./timeWindow";
...
apiRoutes.get("/campaigns", async (req: Request, res: Response) => {
  try {
    ...
    // userId from Clerk middleware (user_xxx format) - matches Campaign.userId
    console.log("[CAMPAIGNS] ✓ GET /api/campaigns - Clerk userId:", userId);
    
    // Filter campaigns by userId (Clerk userId format: user_xxx)
...
apiRoutes.post("/campaigns/:campaignId/start-batch", async (req: Request, res: Response) => {
  try {
    ...
    if (CALL_MODE === "LIVE") {
      ...
      return res.json({ started: true, mode: "DRY_RUN" });
    }
  } catch (err: any) {
